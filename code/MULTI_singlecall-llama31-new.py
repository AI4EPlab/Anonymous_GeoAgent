...
def update_code(code, prompt, solution):
    ...
    if code:
        ...
        prompt_outputs = model.generate(input_prompt_ids, max_new_tokens=256, eos_token_id=terminators, temperature=0.9)
        ...
        messages = [
            ...
            {"role": "user",
             "content": f"\nHere are three suggested code: \n{completion_}."
                        #+ "\nIf the task is completed by previous code, output '#finished' otherwise give your answer."
...
def main():
    ...
    def get_top_k_tokens(curr_node, k=2):
        if curr_node.solution:
            ...
            max_tokens = 700 #random.randint(512, 1024)
    ...
    def beam_search(curr_node, tokens, exec_path):
        print('-----------------------------beam search------------------------')
        temperature = 0.8 #random.uniform(0.7, 1)
        max_tokens = 1024 #random.randint(512, 1536)
        ...
        if code_len == 0:
            ...
            for code_i in unique_lines:
                ...
                if return_back is None:
                    success_len += 1
                elif ('ERROR' not in return_back) and ('Error' not in return_back) and ('EEException' not in return_back):
                    success_len += 1
                else:
                    break
        ...
        if not code.endswith(':\n'):
            try:
                ...
                if return_back:
                    ...
                    elif ('ERROR' in return_back) or ('Error' in return_back) or ('EEException' in return_back):
    ...
    task_path = '/workspace/comparison_bench/cibench_multi/sklearn_exp06.jsonl'
    # this is for cibench
    exec_path_root = '/workspace/comparison_bench/sequential_task/'  #GeoSpatial2task   sequential_task
    base_dir = '/workspace/RAG_bench/ds1000/'
    task_name = os.path.basename(task_path).split('.')[0]
    output_doc = '/workspace/comparison_bench/sequential_task/sklearn_exp06_multi.jsonl'

    tasks = []
    ori_task_id = 0
    with jsonlines.open(task_path, mode='r') as eval_reader:
        samples = [sample for sample in eval_reader][::-1]
    ...
    while tasks:
        ...
        #time.sleep(10)
        # the difference of single and multi-tasks
        if (max_reward == 1) and best_completion:
            pre_tasks = best_completion
        else:
            pre_tasks = ground_truth #pre_tasks + code #current_task['pre_task']
        ...
        if 'code' in current_task:
            ...
            if code not in ground_truth:
                ground_truth = ground_truth + code
        else:
            code = None
        ...
        if 'import ee\n' in pre_tasks and 'ee.Initialize()' not in pre_tasks:
            pre_tasks = pre_tasks.replace('import ee\n', 'import ee\nee.Initialize()\n')
        ...
        write_jsonl(f"{task_name}_multi.jsonl", [item], append=True)
        ...
        render_graphviz_tree(
            root, filename=f"svgviz/tree_{task_name}_{str(task_id)}_multi", view=False
        )
        with open(f"graph_{task_name}_{str(task_id)}_multi.json", "w") as f:
            # print('--------------------------------graph-----------------------------------')
            # print(graph_dict)
            try:
