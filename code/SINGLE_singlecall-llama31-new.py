...
def main():
    ...
    task_path = '/workspace/comparison_bench/GeoOther_multi/selected-sentinel1_timelapse_exe_task.jsonl'
    # this is for cibench
    exec_path_root = '/workspace/comparison_bench/GeoSpatial2task/'  #GeoSpatial2task   sequential_task
    task_name = os.path.basename(task_path).split('.')[0]
    output_doc = '/workspace/comparison_bench/sequential_task/selected-sentinel1_timelapse_exe_task_single.jsonl'
    ...
    while tasks:
        ...
        print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@', reward, completion)
        #time.sleep(10)
        # the difference of single and multi-tasks
        if (reward == 1) and completion:
            pre_tasks = completion
        else:
