...
from multi_process import monitor_process
# 设置监控的最大执行时间和内存使用百分比
...
model_name_path = '/workspace/gemma-2-9b'
...
def model_generate_long_context(curr_node, completions, max_tokens, temperature, num_return_sequences):
    ...
    messages =  '''### Instruction: {}  ### Input: {}  ### Response: {}'''.format("I want you to become my Expert programmer. Your goal is to complete the code.",
...
def main():
    ...
    while tasks:
        ...
        for i in range(max_rollouts):
            ...
            if repeated_task_num > 1 and not child_nodes:
            ...
            if not child_nodes and tokens:
                ...
                completions = [str(idx) + ':' + completion if completion is not None else 'None' for idx, completion in enumerate(top_k_errors)]
                completions = ','.join(completions)
